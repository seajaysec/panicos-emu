...
ROMS="/storage/roms"
# This EmulationStation build only reads es_systems.cfg from /etc (NOT ~/.emulationstation),
# so we layer our systems there — the overlay-layering the PanicOS maintainer sanctioned in
# that file's own comment. /etc is an overlay (writable upper on /storage-base), so this is
# additive and fully reversible: ES_ORIG holds the pristine stock file for restore.
ES_TARGET="/etc/emulationstation/es_systems.cfg"
ES_ORIG="/etc/emulationstation/es_systems.cfg.panicos-orig"
ES_STALE="/storage/.emulationstation/es_systems.cfg"  # earlier (ignored) location — cleaned up
...
gen_es_systems(){
  ...
  echo "     Restore the stock file with: cp $ES_ORIG $ES_TARGET  (or run panicos-emu-uninstall.sh). -->"
  echo '<systemList>'
  # Preserve whatever PanicOS ships (tools, ports, future additions) verbatim, read from the
  # PRISTINE backup so re-runs never compound our own output into the stock set.
  if [ -f "$ES_ORIG" ]; then
    awk '/<system>/{f=1} f{print} /<\/system>/{f=0}' "$ES_ORIG"
  fi
...
render_configs(){
  ...
  # one-time pristine backup of the stock /etc systems (before our first write)
  ...
  rm -f "$ES_STALE" 2>/dev/null || true   # remove the earlier ignored home-dir copy
